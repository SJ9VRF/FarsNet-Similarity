...
public class AdaptedLeskTanimotoNoHyponyms_FN {

	public AdaptedLeskTanimotoNoHyponyms_FN(){
	...
	public double CompMeasure(Vector<FNSynset> fnSynsets_1, Vector<FNSynset> fnSynsets_2){
		...
		for (int i=0; i<fnSynsets_1.size(); i++)
			for (int j=0; j<fnSynsets_2.size(); j++){
				measure = Math.max(measure, lesk.AdaptedLeskTanimoto(fnSynsets_1.elementAt(i), fnSynsets_2.elementAt(j)));
			}
